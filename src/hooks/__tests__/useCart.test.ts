...
describe('useCartStore — addItem', () => {
  it('agrega un item al carrito vacío', async () => {
    ...
    useCartStore.getState().addItem({
      ...
      weightKg: 0.5, imageUrl: null, quantity: 1,
    })
  ...
  it('incrementa quantity si el mismo producto ya está en el carrito', async () => {
    const { useCartStore } = await import('@/hooks/useCart')
    const item = { productId: '1', variantId: null, name: 'Test', unitBasePrice: 1000, weightKg: 0.5, imageUrl: null, quantity: 1 }
  ...
  it('trata products con distinto variantId como items separados', async () => {
    const { useCartStore } = await import('@/hooks/useCart')
    useCartStore.getState().addItem({ productId: '1', variantId: 'A', name: 'Test A', unitBasePrice: 1000, weightKg: 0.5, imageUrl: null, quantity: 1 })
    useCartStore.getState().addItem({ productId: '1', variantId: 'B', name: 'Test B', unitBasePrice: 1000, weightKg: 0.5, imageUrl: null, quantity: 1 })
...
describe('useCartStore — removeItem', () => {
  it('elimina el item por productId+variantId', async () => {
    const { useCartStore } = await import('@/hooks/useCart')
    useCartStore.getState().addItem({ productId: '1', variantId: null, name: 'Test', unitBasePrice: 1000, weightKg: 0.5, imageUrl: null, quantity: 1 })
...
describe('useCartStore — selectores derivados', () => {
  it('itemCount es la suma de quantities', async () => {
    const { useCartStore } = await import('@/hooks/useCart')
    useCartStore.getState().addItem({ productId: '1', variantId: null, name: 'A', unitBasePrice: 1000, weightKg: 0.5, imageUrl: null, quantity: 2 })
    useCartStore.getState().addItem({ productId: '2', variantId: null, name: 'B', unitBasePrice: 2000, weightKg: 1.0, imageUrl: null, quantity: 3 })
